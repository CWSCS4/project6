...
class SymbolMap {
	constructor() {
		this.symbols = new Map()
			...
			.set('SCREEN', 16384)
			.set('KBD', 24576)
		for (let i = 0; i < ALLOCATED_REGISTERS; i++) this.symbols.set('R' + String(i), i)
		this.firstUnusedRegister = ALLOCATED_REGISTERS
	}
	addSymbol(symbol, value) {
		this.symbols.set(symbol, value)
	}
	getSymbol(symbol) {
		const mapValue = this.symbols.get(symbol)
		if (mapValue === undefined) {
			const newValue = this.firstUnusedRegister++
			this.symbols.set(symbol, newValue)
			return newValue
		}
...
class AInstruction {
	...
	resolveSymbol(symbolMap) {
		if (this.isNumber) return
		this.value = symbolMap.getSymbol(this.value)
		this.isNumber = true
	}
	toUint16() {
		return this.value
...
getLines(inStream, line => {
	...
	switch (parseState) {
		...
		case AFTER_LABEL: {
			symbolMap.addSymbol(expression, instructions.length)
			break
		}
		case COMPUTE:
		case JMP_CODE: {
	...
	for (const instruction of instructions) {
		if (instruction instanceof AInstruction) instruction.resolveSymbol(symbolMap)
	}
